 ...
 */
public class MpscUnboundedXaddUnpaddedArrayQueue<E> extends MpUnboundedXaddUnpaddedArrayQueue<MpscUnboundedXaddUnpaddedChunk<E>, E> {
    ...
    @Override
    final MpscUnboundedXaddUnpaddedChunk<E> newChunk(long index, MpscUnboundedXaddUnpaddedChunk<E> prev, int chunkSize, boolean pooled) {
        return new MpscUnboundedXaddUnpaddedChunk(index, prev, chunkSize, pooled);
    }
    ...
    public boolean offer(E e) {
        ...
        MpscUnboundedXaddUnpaddedChunk<E> pChunk = lvProducerChunk();
    ...
    private MpscUnboundedXaddUnpaddedChunk<E> pollNextBuffer(MpscUnboundedXaddUnpaddedChunk<E> cChunk, long cIndex) {
        final MpscUnboundedXaddUnpaddedChunk<E> next = spinForNextIfNotEmpty(cChunk, cIndex);
        if (next == null) {
            return null;
        }
    ...
    private MpscUnboundedXaddUnpaddedChunk<E> spinForNextIfNotEmpty(MpscUnboundedXaddUnpaddedChunk<E> cChunk, long cIndex) {
        MpscUnboundedXaddUnpaddedChunk<E> next = cChunk.lvNext();
        if (next == null) {
    ...
    public E poll() {
        ...
        MpscUnboundedXaddUnpaddedChunk<E> cChunk = this.lvConsumerChunk();
        // start of new chunk?
    ...
    public E peek() {
        ...
        final long cIndex = this.lpConsumerIndex();
        final int ciChunkOffset = (int) (cIndex & chunkMask);
        MpscUnboundedXaddUnpaddedChunk<E> cChunk = this.lpConsumerChunk();
        // start of new chunk?
        if (ciChunkOffset == 0 && cIndex != 0) {
    ...
    public E relaxedPoll() {
        ...
        // start of new chunk?
        if (ciChunkOffset == 0 && cIndex != 0) {
            final MpscUnboundedXaddUnpaddedChunk<E> next = cChunk.lvNext();
            if (next == null) {
                return null;
            }
            e = next.lvElement(0);
    ...
    public int fill(Supplier<E> s, int limit) {
        ...
        MpscUnboundedXaddUnpaddedChunk<E> pChunk = null;
        for (int i = 0; i < limit; i++) {
