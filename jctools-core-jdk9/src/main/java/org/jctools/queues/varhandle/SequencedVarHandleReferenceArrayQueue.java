...
@InternalAPI
public abstract class SequencedVarHandleReferenceArrayQueue<E>
    extends VarHandleReferenceArrayQueue<E> {
  protected final long[] sequenceBuffer;

  public SequencedVarHandleReferenceArrayQueue(int capacity) {
    super(capacity);
    int actualCapacity = this.mask + 1;
    // pad data on either end with some empty slots.
    sequenceBuffer = new long[actualCapacity];
    for (int i = 0; i < actualCapacity; i++) {
      soLongElement(sequenceBuffer, i, i);
    }
  }
