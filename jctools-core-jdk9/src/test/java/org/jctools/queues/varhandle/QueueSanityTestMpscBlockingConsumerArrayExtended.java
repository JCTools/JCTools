...
public class QueueSanityTestMpscBlockingConsumerArrayExtended {
  ...
  public void testOfferPollSemantics() throws Exception {
    ...
    final Queue<Integer> q = new MpscBlockingConsumerArrayQueueVarHandle<>(2);
    // fill up the queue
    while (q.offer(1))
      ;
  ...
  public void testPollTimeout() throws InterruptedException {
    ...
    final MpscBlockingConsumerArrayQueueVarHandle<Object> queue =
        new MpscBlockingConsumerArrayQueueVarHandle<>(128000);
  ...
  public void testOfferBlockingDrainSemantics() throws Exception {
    final AtomicBoolean stop = new AtomicBoolean();
    final AtomicBoolean consumerLock = new AtomicBoolean(true);
    final MpscBlockingConsumerArrayQueueVarHandle<Integer> q =
        new MpscBlockingConsumerArrayQueueVarHandle<>(2);
    // fill up the queue
    while (q.offer(1))
      ;
  ...
  private void testTakeSomeElementsThenBlocksAndIsInterrupted(boolean withTimeout)
      throws Exception {
    Val v = new Val();
    final AtomicBoolean wasInterrupted = new AtomicBoolean();
    final MpscBlockingConsumerArrayQueueVarHandle<Integer> q =
  ...
  /**
   * This test demonstrates a race where a producer wins the CAS and writes null to blocked, only to
   * have the consumer overwrite its change. To have it fail consistently, add a 1ms sleep (or a
   * breakpoint) to parkUntilNext(), just before the soBlocked(Thread.currentThread()). If it hits
   * the race writing to the blocked field, one of the threads will spin forever in
   * spinWaitForUnblock().
   */
  @Test(timeout = TEST_TIMEOUT)
