...
public final class JavaParsingAtomicArrayQueueGenerator extends VoidVisitorAdapter<Void> {
    ...
    public void visit(ClassOrInterfaceDeclaration node, Void arg) {
        ...
        if (isCommentPresent(node, "$gen:ordered-fields")) {
        ...
        for (MethodDeclaration method : node.getMethods()) {
            if (isCommentPresent(method, "$gen:ignore")) {
                method.remove();
            }
        }
    ...
    private static boolean isRefArray(Type in) {
        if (in instanceof ArrayType) {
            ArrayType aType = (ArrayType) in;
            return aType.getComponentType() instanceof ClassOrInterfaceType;
        }
        return false;
    }
    ...
    private static void organiseImports(CompilationUnit cu) {
        ...
        for (ImportDeclaration importDeclaration : cu.getImports()) {
            if (importDeclaration.getNameAsString().startsWith("org.jctools.util.Unsafe")) {
                System.out.println("Ignoring import " + importDeclaration.getNameAsString());
                continue;
            }
    ...
    private static void processSpecialNodeTypes(NodeWithType<?, Type> node, String name) {
        ...
        if ("buffer".equals(name) && isRefArray(type)) {
            ...
            node.setType(atomicLongArrayType());
                ...
                && ("mask".equals(name) || "offset".equals(name) || "seqOffset".equals(name))) {
