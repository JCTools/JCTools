 ...
 * which can found in the jctools-build module. The original source file is MpscUnboundedXaddArrayQueue.java.
 *
 ...
 */
public class MpscUnboundedXaddVarHandleUnpaddedArrayQueue<E> extends MpUnboundedXaddVarHandleUnpaddedArrayQueue<MpscUnboundedXaddVarHandleUnpaddedChunk<E>, E> {
     ...
     */
    public MpscUnboundedXaddVarHandleUnpaddedArrayQueue(int chunkSize, int maxPooledChunks) {
        super(chunkSize, maxPooledChunks);
    }

    public MpscUnboundedXaddVarHandleUnpaddedArrayQueue(int chunkSize) {
        this(chunkSize, 2);
    }

    @Override
    final MpscUnboundedXaddVarHandleUnpaddedChunk<E> newChunk(long index, MpscUnboundedXaddVarHandleUnpaddedChunk<E> prev, int chunkSize, boolean pooled) {
        return new MpscUnboundedXaddVarHandleUnpaddedChunk(index, prev, chunkSize, pooled);
    }
    ...
    public boolean offer(E e) {
        ...
        MpscUnboundedXaddVarHandleUnpaddedChunk<E> pChunk = lvProducerChunk();
    ...
    private MpscUnboundedXaddVarHandleUnpaddedChunk<E> pollNextBuffer(MpscUnboundedXaddVarHandleUnpaddedChunk<E> cChunk, long cIndex) {
        final MpscUnboundedXaddVarHandleUnpaddedChunk<E> next = spinForNextIfNotEmpty(cChunk, cIndex);
        if (next == null) {
            return null;
        }
    ...
    private MpscUnboundedXaddVarHandleUnpaddedChunk<E> spinForNextIfNotEmpty(MpscUnboundedXaddVarHandleUnpaddedChunk<E> cChunk, long cIndex) {
        MpscUnboundedXaddVarHandleUnpaddedChunk<E> next = cChunk.lvNext();
        if (next == null) {
    ...
    public E poll() {
        ...
        MpscUnboundedXaddVarHandleUnpaddedChunk<E> cChunk = this.lvConsumerChunk();
        // start of new chunk?
    ...
    public E peek() {
        ...
        final long cIndex = this.lpConsumerIndex();
        final int ciChunkOffset = (int) (cIndex & chunkMask);
        MpscUnboundedXaddVarHandleUnpaddedChunk<E> cChunk = this.lpConsumerChunk();
        // start of new chunk?
        if (ciChunkOffset == 0 && cIndex != 0) {
    ...
    public E relaxedPoll() {
        ...
        // start of new chunk?
        if (ciChunkOffset == 0 && cIndex != 0) {
            final MpscUnboundedXaddVarHandleUnpaddedChunk<E> next = cChunk.lvNext();
            if (next == null) {
                return null;
            }
            e = next.lvElement(0);
    ...
    public int fill(Supplier<E> s, int limit) {
        ...
        MpscUnboundedXaddVarHandleUnpaddedChunk<E> pChunk = null;
        for (int i = 0; i < limit; i++) {
