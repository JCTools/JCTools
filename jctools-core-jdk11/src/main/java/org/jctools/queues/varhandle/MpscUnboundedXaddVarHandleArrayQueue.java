 ...
 */
public class MpscUnboundedXaddVarHandleArrayQueue<E> extends MpUnboundedXaddVarHandleArrayQueue<MpscUnboundedXaddVarHandleChunk<E>, E> {
     ...
     */
    public MpscUnboundedXaddVarHandleArrayQueue(int chunkSize, int maxPooledChunks) {
        super(chunkSize, maxPooledChunks);
    }

    public MpscUnboundedXaddVarHandleArrayQueue(int chunkSize) {
        this(chunkSize, 2);
    }

    @Override
    final MpscUnboundedXaddVarHandleChunk<E> newChunk(long index, MpscUnboundedXaddVarHandleChunk<E> prev, int chunkSize, boolean pooled) {
        return new MpscUnboundedXaddVarHandleChunk(index, prev, chunkSize, pooled);
    }
    ...
    public boolean offer(E e) {
        ...
        MpscUnboundedXaddVarHandleChunk<E> pChunk = lvProducerChunk();
    ...
    private MpscUnboundedXaddVarHandleChunk<E> pollNextBuffer(MpscUnboundedXaddVarHandleChunk<E> cChunk, long cIndex) {
        final MpscUnboundedXaddVarHandleChunk<E> next = spinForNextIfNotEmpty(cChunk, cIndex);
        if (next == null) {
            return null;
        }
    ...
    private MpscUnboundedXaddVarHandleChunk<E> spinForNextIfNotEmpty(MpscUnboundedXaddVarHandleChunk<E> cChunk, long cIndex) {
        MpscUnboundedXaddVarHandleChunk<E> next = cChunk.lvNext();
        if (next == null) {
    ...
    public E poll() {
        ...
        MpscUnboundedXaddVarHandleChunk<E> cChunk = this.lvConsumerChunk();
        // start of new chunk?
    ...
    public E peek() {
        ...
        final long cIndex = this.lpConsumerIndex();
        final int ciChunkOffset = (int) (cIndex & chunkMask);
        MpscUnboundedXaddVarHandleChunk<E> cChunk = this.lpConsumerChunk();
        // start of new chunk?
        if (ciChunkOffset == 0 && cIndex != 0) {
    ...
    public E relaxedPoll() {
        ...
        // start of new chunk?
        if (ciChunkOffset == 0 && cIndex != 0) {
            final MpscUnboundedXaddVarHandleChunk<E> next = cChunk.lvNext();
            if (next == null) {
                return null;
            }
            e = next.lvElement(0);
    ...
    public int fill(Supplier<E> s, int limit) {
        ...
        MpscUnboundedXaddVarHandleChunk<E> pChunk = null;
        for (int i = 0; i < limit; i++) {
